...
def rot(st, n):
    return "".join(
        chr((ord(i) - n - 97 + 26) % 26 + 97) if i.isalpha() and i.islower() else i
        for i in st
    )
...
def main():
    ...
    for n in range(26):
        print(n, rot("NCKUCTF{elcelc_o@H@H4_w@ukh_1u1u1u_Cexii3kd3!", n))

        # challenge_message = get_challenge_message(n)
