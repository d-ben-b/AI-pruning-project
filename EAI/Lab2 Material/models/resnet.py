...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, planes, out_channels, downsample=None, stride=1):
        ...
        assert (
            len(out_channels) == 3
        ), "Bottleneck requires out_channels list of length 3"
    ...
    def forward(self, x):
        ...
        if verbose:
            ...
            print(f"[DEBUG] identity before downsample = {identity.shape}")
        ...
        if self.downsample is not None:
            if verbose:
                print(f"[DEBUG] before downsample = {identity.shape}")
            identity = self.downsample(identity)
            if verbose:
                print(f"[DEBUG] after downsample = {identity.shape}")

        out += identity
        if verbose:
            print(f"[DEBUG] add done, out shape = {out.shape}")
        out = self.relu(out)
        return out
...
class ResNet(nn.Module):
    def __init__(self, block, layers, cfg, num_classes=1000, in_channels=3):
        ...
        if verbose:
            print("Total cfg used:", self.current_cfg_idx)
        # Conv1
        self.conv1 = nn.Conv2d(
    ...
    def _make_layer(self, block, planes, blocks, cfg, stride=1):
        ...
        # === Step 1. Downsample 處理 ===
        downsample = None
        if stride != 1 or self.inplanes != out_channels[-1]:
            # print(f"[DEBUG] output channel shape = {out_channels[-1]}")
        ...
        # === Step 2. 第一個 Bottleneck（Projection Shortcut）===
        layers.append(
            block(
...
def ResNet50(num_classes=10, in_channels=3, cfg=None):
    ...
    layers = [3, 4, 6, 3]
    return ResNet(
        Bottleneck, layers, cfg, num_classes=num_classes, in_channels=in_channels
    )
