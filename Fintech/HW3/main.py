...
def point_to_hex(point):
    ...
    if point == ellipticcurve.INFINITY:
        return "INFINITY"
    return (
        f"{point.x().to_bytes(32, 'big').hex()} {point.y().to_bytes(32, 'big').hex()}"
    )
...
def Problem4(indata):
    ...
    result_dG, num_doubles, num_additions = double_and_add(
        int(indata), getG(), getINFINITY
    )
...
def Problem5(indata):
    ...
    opt_result_dG, opt_num_doubles, opt_num_additions = optimized_double_and_add(
        int(indata), getG(), getINFINITY
    )
...
def Problem6(private_key, transaction_hashID):
    ...
    signature = sign_transaction(
        private_key, transaction_hashID, getG, getN, random.randint
    )
...
def Problem7(private_key, indata):
    ...
    is_valid = verify_signature(
        public_key, transaction_hashID, signature, getG, getN, getINFINITY
    )
    print(f"{is_valid}")
